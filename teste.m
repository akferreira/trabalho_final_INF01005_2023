...
ber_base = zeros(size(Eb_N0_lin)); %pré-alocação do vetor de BER
ber_reed = zeros(size(Eb_N0_lin)); %pré-alocação do vetor de BER
...
num_b = bps*K*log2(N+1)*250; %número de bits a serem simulados
...
modData = pskmod(data);
%chunk_modData = reshape(modData,chunk_size,[]);
%chunk_data  = reshape(data,chunk_size*bps,[]);

encodedData = rsEncoder(data);
encodedModData = pskmod(encodedData);

%ber
for i = 1:length(Eb_N0_lin)
    n = NA_8psk_base(i)*complex(randn(1, num_b/bps), randn(1, num_b/bps))*sqrt(0.5);
    recebido = modData + n'; 
    demod = pskdemod(recebido);
    errors = sum(demod ~= data);
    ber_base(i) = errors / num_b; % contagem de erros e cálculo do BER
end
...
for i = 1:length(Eb_N0_lin)
    ...
    n = NA_8psk_RS(i)*complex(randn(1, len_noise), randn(1, len_noise))*sqrt(0.5); %vetor de ruído complexo com desvio padrão igual a uma posição do vetor NA
    recebido = encodedModData+n';
    demod = pskdemod(recebido); 
    decoded = rsDecoder(demod);
    errors = sum(decoded ~= data);
    ber_reed(i) = errors / num_b; % contagem de erros e cálculo do BER
end
